<script lang="ts">
    ...
    let transferTargetUserId: number | null = null;
    let transferNewRole = "";
    ...
    onMount(async () => {
        if (browser) {
            ...
            if (!token) {
                ...
                await loadClass();
                await Promise.all([
                    loadAnnouncements(announcementSort),
                    loadTasks(),
                ]);
            }
        }
    ...
    function updateGradesFromTasks() {
        ...
        grades = withGrades.map((task) => {
            ...
            const percentage = Number.isNaN(gradeNumber)
                ? "-"
                : `${Math.round((gradeNumber / 10) * 100)}%`;
            return {
                task: task.title,
                grade: task.grade,
                maxGrade: "10",
        ...
        const total = grades.reduce(
            (acc, item) => acc + Number(item.grade || 0),
            0,
        );
    ...
    function hydrateClassMembers() {
        students = [];
        teachers = [];
        const members = Array.isArray(clase.students_info)
            ? clase.students_info
            : [];
        for (let i = 0; i < members.length; i++) {
            ...
            if (members[i].role === "student") {
                ...
                members[i].role === "teacher" ||
                members[i].role === "assistant"
            ) {
    ...
    async function loadClass() {
        ...
        const response = await fetchWithRateLimit(
            `${urlip}class/obtainClassByID/${id}`,
            {
                method: "get",
                headers: {
        ...
        if (response.ok) {
            ...
            showAlert(
                "Error",
                data?.Error || "No se pudo cargar la clase",
                "red",
            );
        }
    }
    ...
    async function loadAnnouncements(sortMode = announcementSort) {
        ...
        const token = localStorage.getItem("token");
        const response = await fetchWithRateLimit(
            `${urlip}class/obtainAnnouncements/${id}/?sort=${encodeURIComponent(sortMode)}`,
            {
                method: "get",
                headers: {
                    "Content-Type": "application/json",
        ...
        if (response.ok) {
            ...
            showAlert(
                "Error",
                data?.Error || "No se pudieron cargar los anuncios",
                "red",
            );
        }
    }
    ...
    async function createNews() {
        ...
        isSubmitting = true;
        const response = await fetchWithRateLimit(
            `${urlip}class/createAnnouncement/`,
            {
                method: "POST",
                headers: {
                    authorization: `${token}`,
        ...
        if (response.ok) {
            ...
            showAlert(
                "Error",
                data?.Error || "No se pudo crear el anuncio",
                "red",
            );
        }
    }
    ...
    async function loadTasks() {
        ...
        try {
            const response = await fetchWithRateLimit(
                `${urlip}class/obtainPendingTasks/${id}/`,
                {
                    method: "GET",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (!response.ok) {
                showAlert(
                    "Error",
                    data?.Error || "No se pudieron cargar las tareas",
                    "red",
                );
                tasks = [];
                return;
    ...
    async function submitTask(taskId: number) {
        ...
        if (!files.length) {
            showAlert(
                "Error",
                "Selecciona al menos un archivo para entregar",
                "orange",
            );
            return;
        }
        ...
        if (!response.ok) {
            showAlert(
                "Error",
                data?.Error || "No se pudo entregar la tarea",
                "red",
            );
            return;
        }
    ...
    function openEditAnnouncement(announcement: any) {
        ...
        editNewsUrls = Array.isArray(announcement.urls)
            ? announcement.urls.join("\n")
            : "";
        showEditNews = true;
    }
    ...
    async function saveEditedAnnouncement() {
        ...
        const response = await fetchWithRateLimit(
            `${urlip}class/manageAnnouncement/${editingAnnouncementId}/`,
            {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
        ...
        if (!response.ok) {
            showAlert(
                "Error",
                data?.Error || "No se pudo editar el anuncio",
                "red",
            );
            return;
        }
    ...
    async function deleteAnnouncement(announcementId: number) {
        ...
        const token = localStorage.getItem("token");
        const response = await fetchWithRateLimit(
            `${urlip}class/manageAnnouncement/${announcementId}/`,
            {
                method: "DELETE",
                headers: {
                    authorization: `${token}`,
    ...
    async function saveEditedTask() {
        ...
        if (!response.ok) {
            showAlert(
                "Error",
                data?.Error || "No se pudo editar la tarea",
                "red",
            );
            return;
        }
    ...
    async function deleteTask(taskId: number) {
        if (
            !confirm(
                "¿Quieres eliminar esta tarea? También se eliminarán sus entregas.",
            )
        )
            return;
    ...
    async function expulsarAlumno(studentId: number) {
        if (!confirm("¿Seguro que quieres expulsar a este alumno de la clase?"))
            return;
        const token = localStorage.getItem("token");
        const response = await fetchWithRateLimit(
            `${urlip}class/removeMember/${id}/${studentId}/`,
            {
                method: "DELETE",
                headers: {
                    authorization: `${token}`,
        ...
        if (!response.ok) {
            showAlert(
                "Error",
                data?.Error || "No se pudo expulsar al alumno",
                "red",
            );
            return;
        }
    ...
    async function abandonarClase() {
        ...
        if (!response.ok) {
            showAlert(
                "Error",
                data?.Error || "No se pudo abandonar la clase",
                "red",
            );
            return;
        }
    ...
    function handleRoleChange(
        ...
        if (newRole === "teacher" && isClassOwner && userId !== currentUserId) {
            transferTargetUserId = userId;
            transferNewRole = newRole;
            transferSelectElement = select;
            transferPreviousValue = currentRole;
            showTransferOwnershipModal = true;
        } else {
            actualizarRolMiembro(userId, newRole);
        }
    }

    function confirmTransferOwnership() {
        if (transferTargetUserId !== null && transferNewRole) {
            actualizarRolMiembro(transferTargetUserId, transferNewRole);
        }
        closeTransferModal();
    }

    function cancelTransferOwnership() {
        if (transferSelectElement) {
            transferSelectElement.value = transferPreviousValue;
        }
        closeTransferModal();
    }
    ...
    function closeTransferModal() {
        ...
        transferNewRole = "";
        transferSelectElement = null;
        transferPreviousValue = "";
    }
</script>
        ...
        <div class="card" style="width: 90%; max-width: 500px;">
            ...
            <div style="padding: 16px;">
                ...
                <div style="margin-bottom: 20px;">
                    <label
                        style="display: block; margin-bottom: 8px; color: var(--text-color);"
...
{#if showTransferOwnershipModal}
    <div class="modal-overlay" on:click={cancelTransferOwnership}>
        <div class="modal-card" on:click|stopPropagation>
            <button
                class="modal-close"
                on:click={cancelTransferOwnership}
                aria-label="Cerrar">×</button
            >
            <div class="form-header">
                <h2>Transferir propiedad de la clase</h2>
                <p>
            ...
            <div class="form-actions" style="margin-top: 20px;">
                <button class="cancel-btn" on:click={cancelTransferOwnership}
                    >Cancelar</button
                >
                <button class="send-btn" on:click={confirmTransferOwnership}
                    >Aceptar</button
                >
            </div>
        </div>
            ...
            <form on:submit|preventDefault={saveEditedAnnouncement}>
                ...
                <div class="form-group">
                    ...
                    <textarea
                        ...
                        rows="4"
                        bind:value={editNewsDescription}
                        required
                    ></textarea>
                </div>
                <div class="form-group">
                ...
                <div class="form-actions">
                    <button
                        type="button"
                        class="cancel-btn"
                        on:click={() => (showEditNews = false)}>Cancelar</button
                    >
    ...
    <div class="modal-overlay" on:click={() => (showEditTask = false)}>
        <div class="modal-card" on:click|stopPropagation>
            ...
            <form on:submit|preventDefault={saveEditedTask}>
                ...
                {#if !editTaskAllowAnyFileType}
                    <div class="form-group">
                        <label for="editTaskAllowedExtensions"
                            >Extensiones permitidas</label
                        >
                        <input
                            id="editTaskAllowedExtensions"
                            type="text"
                ...
                <div class="form-actions">
                    <button
                        type="button"
                        class="cancel-btn"
                        on:click={() => (showEditTask = false)}>Cancelar</button
                    >
                    <button class="send-btn" type="submit"
                        >Guardar cambios</button
                    >
...
<div class="class-container">
    ...
    <div class="class-tabs">
        <button
            class="tab-button"
            class:active={activeTab === "tablón"}
            on:click={() => (activeTab = "tablón")}>Tablón</button
        >
        <button
            class="tab-button"
        ...
        <button
            ...
            on:click={() => (activeTab = "personas")}>Personas</button
        >
        <button
            class="tab-button"
        ...
        <div class="main-content">
            {#if activeTab === "tablón"}
                <div class="card-title">
                    ...
                    <div class="manage-row">
                        ...
                        <button
                            on:click={() => (showCreateNews = true)}
                            class="action-button">+ Nuevo anuncio</button
                        >
                    </div>
                </div>
                    ...
                    {#each announcements as announcement}
                        <div class="announcement-item">
                            <div class="announcement-header">
                                <div class="avatar">
                                    {announcement.creator_info?.first_name?.charAt(
                                        0,
                                    )}{announcement.creator_info?.last_name?.charAt(
                                        0,
                                    )}
                                </div>
                                <div class="announcement-meta">
                                    <div class="announcement-author">
                                        {announcement.creator_info?.first_name}
                                        {announcement.creator_info?.last_name}
                                    </div>
                                    <div class="announcement-date">
                                        {announcement.created_at} • {announcement.comments_count ||
                                            0} comentario(s)
                                    </div>
                                </div>
                                <div class="manage-row">
                                    <a
                                        class="secondary-button"
                                        href={`/clases/clase-${id}/anuncio-${announcement.id}`}
                                        >Ver hilo</a
                                    >
                                    {#if announcement.can_edit}
                                        <button
                                            class="secondary-button manage-btn"
                                            type="button"
                                            on:click={() =>
                                    ...
                                    {#if announcement.can_delete}
                                        <button
                                            class="secondary-button manage-btn danger-btn"
                                            type="button"
                                            on:click={() =>
                                                deleteAnnouncement(
                                                    announcement.id,
                    ...
                    {#each tasks as task}
                        <div class="task-item task-item-expanded">
                            ...
                            <div class="task-meta-row">
                                {#if areYouTeacher}
                                    <span class="task-due"
                                        >Entregadas: {task.delivered_count ??
                                            0}</span
                                    >
                                    <span class="task-due"
                                        >Pendientes: {task.pending_count ??
                                            0}</span
                                    >
                                    <span class="task-due"
                                        >Por calificar: {task.to_grade_count ??
                                            0}</span
                                    >
                                {:else}
                                    <span class="task-due"
                                        >Nota: {task.grade ?? "-"}</span
                                    >
                                {/if}
                                <a
                                    class="secondary-button"
            ...
            {#if activeTab === "calificaciones"}
                ...
                {#if areYouTeacher}
                    <div class="announcement-item">
                        <div class="announcement-title">
                            Esta vista de calificaciones es para alumnos.
                        </div>
                    </div>
                    ...
                    <div class="material-item">
                        ...
                        <div class="material-info">
                            ...
                            <div class="material-meta">
                                {material.size} • Subido el {material.date}
                            </div>
                        </div>
                    </div>
                {/each}
        ...
        <div class="sidebar">
            ...
            <div class="card">
                ...
                <button
                    class="secondary-button"
                    style="width: 100%; margin-bottom: 8px;"
                    on:click={abandonarClase}
                >
                    Abandonar clase
                </button>
                {#if areYouTeacher}
                    <a
                        href="/clases/clase-{id}/dashboard"
                        class="secondary-button"
                        style="width: 100%;"
                    >
                        Dashboard
