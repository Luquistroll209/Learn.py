<script lang="ts">
    ...
    function buildThreadComments(rawComments: any[], sortMode: string): any[] {
        ...
        const sortByCreatedAsc = (a: any, b: any) => dateToMillis(a.created_at) - dateToMillis(b.created_at);
        const sortByCreatedDesc = (a: any, b: any) => dateToMillis(b.created_at) - dateToMillis(a.created_at);
        ...
        if (sortMode === 'new') {
            ...
            roots.sort((a: any, b: any) => {
                const repliesDiff = Number(b.replies_count || 0) - Number(a.replies_count || 0);
    ...
    async function loadAnnouncement() {
        ...
        loadError = '';
        try {
            const response = await fetch(`${urlip}class/obtainAnnouncementDetail/${id}/`, {
                method: 'GET',
                headers: {
    ...
    async function createComment(parentId: number | null = null) {
        ...
        const response = await fetch(`${urlip}class/createAnnouncementComment/${id}/`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                authorization: `${token}`
    ...
    async function saveEditedComment() {
        ...
        const response = await fetch(`${urlip}class/manageAnnouncementComment/${editingCommentId}/`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                authorization: `${token}`
        ...
        const data = await response.json();
        if (!response.ok) {
            showAlert('Error', data?.Error || 'No se pudo editar el comentario', 'red');
            return;
        }
    ...
    async function deleteComment(commentId: number) {
        ...
        const response = await fetch(`${urlip}class/manageAnnouncementComment/${commentId}/`, {
            method: 'DELETE',
            headers: {
                authorization: `${token}`
            }
        ...
        const data = await response.json();
        if (!response.ok) {
            showAlert('Error', data?.Error || 'No se pudo eliminar el comentario', 'red');
            return;
        }
        await loadAnnouncement();
    }
    ...
    async function deleteAnnouncement() {
        ...
        const response = await fetch(`${urlip}class/manageAnnouncement/${announcement.id}/`, {
            method: 'DELETE',
            headers: {
                authorization: `${token}`
            }
...
<div class="class-container forum-page">
    ...
    {:else if loadError}
        <div class="main-content task-state-card">
            <p>{loadError}</p>
            <button class="secondary-button" on:click={loadAnnouncement}>Reintentar</button>
        </div>
        ...
        <div class="task-layout-grid forum-layout">
            <section class="main-content classroom-main">
                <article class="card forum-post">
                    <div class="task-title-row">
                        <div class="task-icon-circle"><i class="fa-solid fa-comments"></i></div>
                        <div class="task-title-block">
                            <h1>{announcement.title}</h1>
                            <p>{displayName(announcement.creator_info)} • publicado {formatDate(announcement.created_at)}</p>
                        </div>
                        {#if classId}
                            <a class="secondary-button task-back-link" href={`/clases/clase-${classId}`}>Volver</a>
                        {/if}
                    </div>

                    <div class="task-divider"></div>
                    <div class="task-description forum-post-body">{announcement.description || 'Sin descripción.'}</div>
                ...
                <section class="card forum-comments-block">
                    <div class="forum-comments-header">
                        <div class="card-title">Discusión ({comments.length})</div>
                        <select class="forum-sort-select" bind:value={commentsSort}>
                    ...
                    <div class="forum-new-comment">
                        <textarea rows="3" bind:value={newComment} placeholder="Comparte tu opinión..."></textarea>
                        <button class="action-button" on:click={() => createComment(null)} disabled={isSendingComment}>
                            {isSendingComment ? 'Publicando...' : 'Publicar comentario'}
                        </button>
                    </div>

                    {#if threadComments.length === 0}
                        <div class="student-work-empty" style="margin-top: 12px;">Sé el primero en comentar.</div>
                    {:else}
                        <div class="forum-thread-list">
                            {#each threadComments as comment (comment.id)}
                                <article class="forum-comment" style:margin-left={`${Math.min(comment.depth, 6) * 24}px`}>
                                    <div class="forum-comment-head">
                                        <div class="announcement-author">{displayName(comment.author_info)}</div>
                                        <div class="forum-comment-meta">
                                            <span>{formatDate(comment.created_at)}</span>
                                            {#if Number(comment.replies_count || 0) > 0}
                                                <span>• {comment.replies_count} respuesta(s)</span>
                                            {/if}
                                        </div>
                                    </div>
                                    ...
                                    {#if editingCommentId === comment.id}
                                        <div class="form-group" style="margin-top: 8px;">
                                            <textarea rows="3" bind:value={editingCommentContent}></textarea>
                                        </div>
                                        <div class="forum-comment-actions">
                                            <button class="secondary-button" on:click={() => { editingCommentId = null; editingCommentContent = ''; }}>Cancelar</button>
                                            <button class="action-button" on:click={saveEditedComment}>Guardar</button>
                                        </div>
                                    {:else}
                                        <div class="announcement-content forum-comment-content">{comment.content}</div>
                                        <div class="forum-comment-actions">
                                            <button class="secondary-button" on:click={() => { replyingToCommentId = replyingToCommentId === comment.id ? null : comment.id; }}>
                                                Responder
                                            </button>
                                            {#if comment.can_edit}
                                                <button class="secondary-button" on:click={() => startEditComment(comment)}>Editar</button>
                                            {/if}
                                            {#if comment.can_delete}
                                                <button class="secondary-button forum-danger" on:click={() => deleteComment(comment.id)}>Eliminar</button>
                                            {/if}
                                        </div>
                                    {/if}
                                    ...
                                    {#if replyingToCommentId === comment.id}
                                        <div class="forum-reply-box">
                                            <textarea
                                                rows="2"
                                                bind:value={replyDraftByComment[comment.id]}
                                                placeholder="Responder a este comentario..."
                                            ></textarea>
                                            <div class="forum-comment-actions">
                                                <button class="secondary-button" on:click={() => { replyingToCommentId = null; replyDraftByComment = { ...replyDraftByComment, [comment.id]: '' }; }}>
                                                    Cancelar
                                                </button>
                                                <button class="action-button" on:click={() => createComment(comment.id)} disabled={isSendingComment}>
                                                    Responder
                                                </button>
                                            </div>
                                        </div>
            ...
            <aside class="sidebar classroom-sidebar">
                <div class="card">
                    ...
                    <div class="task-due"><strong>Comentarios:</strong> {comments.length}</div>
                    <div class="task-due"><strong>Última actividad:</strong> {formatDate(announcement.last_activity_at || announcement.updated_at || announcement.created_at)}</div>
                </div>

                {#if canDeleteAnnouncement}
                    <div class="card">
                        <div class="card-title">Gestión</div>
                        <button class="secondary-button forum-danger" on:click={deleteAnnouncement}>
                            Eliminar anuncio
                        </button>
                    </div>
                {/if}
