<script lang="ts">
    ...
    async function handleAuthSubmit(event: Event): Promise<void> {
        ...
        const email = (form.querySelector("#login-email") as HTMLInputElement)
            .value;
        const password = (
            form.querySelector("#login-password") as HTMLInputElement
        ).value;

        try {
            const respose = await fetchWithRateLimit(`${urlip}users/login/`, {
                method: "post",
                headers: {
...
<form class="auth-form" on:submit={handleAuthSubmit}>
    ...
    <div class="auth-switch">
        ...
        <a href="/auth/register" class="auth-switch-link"> Regístrate aquí </a>
    </div>
</form>
