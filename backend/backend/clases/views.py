...
def get_membership(user, clase):
    return (
        ClaseMembership.objects.filter(user=user, clase=clase)
        .select_related("user")
        .first()
    )
...
def get_task_submissions_by_student(task):
    submissions = task.submissions.select_related("student").prefetch_related("files")
...
def build_teacher_task_participants(task, request=None):
    memberships = (
        ClaseMembership.objects.filter(clase=task.clase)
        .select_related("user")
        .order_by("joined_at")
    )
...
def get_pending_tasks_queryset(clase):
    now = timezone.now()
    return (
        Task.objects.filter(clase=clase)
...
def build_user_classes_overview(user, request=None):
    memberships = list(
        ClaseMembership.objects.filter(user=user)
        .select_related("clase", "clase__teacher")
        .order_by("-joined_at")
    )

    membership_by_class_id = {
        membership.clase_id: membership for membership in memberships
    }
    owned_without_membership = list(
        Clase.objects.filter(teacher=user)
    ...
    classes = [
        membership.clase for membership in memberships
    ...
    role_by_class_id = {
        membership.clase_id: membership.role for membership in memberships
    }
    for clase in owned_without_membership:
        role_by_class_id[clase.id] = "teacher"

    classes_serialized = ClaseSerializer(
    ...
    now = timezone.now()
    all_tasks = list(
        Task.objects.filter(clase_id__in=class_ids)
    ...
    user_submissions = list(
        TaskSubmission.objects.filter(
            task_id__in=task_ids, student=user
        ).select_related("task")
    )
    user_submission_by_task_id = {
        submission.task_id: submission for submission in user_submissions
    }

    graded_submissions = list(
        TaskSubmission.objects.filter(
            task_id__in=task_ids, grade__isnull=False
        ).select_related("task")
    )
    class_grade_totals = {}
    class_grade_counts = {}
    for submission in graded_submissions:
        ...
        class_grade_totals[class_id] = class_grade_totals.get(
            class_id, Decimal("0")
        ) + Decimal(submission.grade)
    ...
    for clase in classes:
        ...
        if role in ("teacher", "assistant"):
            open_tasks = [
                task for task in class_tasks if not task.due_at or task.due_at >= now
            ]
    ...
    recent_activity = []
    announcements = (
        Announcement.objects.filter(clase_id__in=class_ids)
        .select_related("clase")
    ...
    user_submissions_recent = (
        TaskSubmission.objects.filter(task_id__in=task_ids, student=user)
    ...
    if overall_grade_count:
        average_grade = grade_average_to_response(
            overall_grade_total, overall_grade_count
        )
...
def build_user_tasks_overview(user, request=None, class_filter=None):
    ...
    for task in tasks:
        ...
        if role in ("teacher", "assistant"):
            ...
            grade_value = (
                grade_to_response_value(submission.grade) if submission else None
            )
...
def build_class_dashboard_payload(clase):
    memberships = list(
        ClaseMembership.objects.filter(clase=clase)
        .select_related("user")
        .order_by("joined_at")
    )
    ...
    submissions = list(
        TaskSubmission.objects.filter(task__in=tasks).select_related("task", "student")
    )
    submissions_by_task = {}
        ...
        task_rows.append(
            {
                ...
                "average_grade": grade_average_to_response(
                    task_grade_total, task_grade_count
                ),
                "created_at": task.created_at,
            }
        )
    ...
    for membership in students:
        ...
        if average_grade is not None:
            average_percentage = str(
                int(round((Decimal(average_grade) / Decimal("10")) * 100))
            )
    ...
    return {
        ...
        "stats": {
            ...
            "class_average_grade": grade_average_to_response(
                class_grade_total, class_grade_count
            ),
        },
    }
...
class CreateClassView(APIView):
    def post(self, request):
        ...
        # No copiar todo request.data directamente
        # En lugar de eso, trabajar con request.data directamente

        # Si necesitas modificar los datos, crea un diccionario mutable
        data = {}

        # Copiar solo los campos que necesitas (los que no son archivos)
        if "name" in request.data:
            data["name"] = request.data.get("name")
        if "description" in request.data:
            data["description"] = request.data.get("description")

        # Manejar el archivo por separado
        if "imagen" in request.FILES:
            data["imagen"] = request.FILES.get("imagen")

        serializer = ClaseSerializer(
            data=data, context={"request": request, "teacher": user}
        )
...
class ObtainUserTasksOverviewView(APIView):
    def get(self, request):
        ...
        data = build_user_tasks_overview(
            user, request=request, class_filter=class_filter
        )
...
class obtainClassByID(APIView):
    def get(self, request, identifier=None):
        ...
        if not user:
            return Response(
                {"Error": "Token incorrecto"}, status=status.HTTP_401_UNAUTHORIZED
            )
        ...
        if not clase:
            return Response(
                {"Error": "Clase no encontrada"}, status=status.HTTP_404_NOT_FOUND
            )
        membership = ClaseMembership.objects.filter(
            user=user, clase_id=identifier
        ).first()
...
class ObtainClassDashboardView(APIView):
    def get(self, request, clase_id=None):
        ...
        if not clase_id:
            return Response(
                {"Error": "clase_id es requerido"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            if clase.teacher != user:
                return Response(
                    {"Error": "Solo el profesor principal puede invitar usuarios"},
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            if clase.students.filter(id=user_to_invite.id).exists():
                return Response(
                    {"Error": "El usuario ya está en esta clase"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        ...
        try:
            ...
            if clase.students.filter(id=user.id).exists():
                ...
                student_info = next(
                    (
                        s
                        for s in serializer.data["students_info"]
                ...
                return Response(
                    {
                        ...
                        "unionDate": student_info["joined_at"]
                        if student_info
                        else None,
...
class UpdateClassSettingsView(APIView):
    ...
    def patch(self, request, clase_id):
        ...
        serializer = ClaseSerializer(clase, context={"request": request})
        return Response(
            {"message": "Clase actualizada correctamente", "clase": serializer.data},
            status=status.HTTP_200_OK,
        )
...
class RemoveClassMemberView(APIView):
    def delete(self, request, clase_id, user_id):
        ...
        try:
            member = ClaseMembership.objects.select_related("user").get(
                clase=clase, user_id=user_id
            )
        ...
        if member.role == "teacher" or member.user_id == clase.teacher_id:
            return Response(
                {"Error": "No puedes expulsar al profesor"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class UpdateClassMemberRoleView(APIView):
    def patch(self, request, clase_id, user_id):
        ...
        if new_role == "teacher":
            ...
            old_teacher_membership = ClaseMembership.objects.filter(
                clase=clase, user_id=clase.teacher_id
            ).first()
...
class CreateAnnouncementView(APIView):
    ...
    def post(self, request):
        ...
        if not raw_data.get("title"):
            return Response(
                {"Error": "title es requerido"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        urls_list = [
            item for item in parse_list(raw_data.get("urls")) if isinstance(item, str)
        ]
        existing_photos = [
            item for item in parse_list(raw_data.get("photos")) if isinstance(item, str)
        ]
        ...
        for image in uploaded_files:
            try:
                ...
                return Response({"Error": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
            photos_urls.append(
                request.build_absolute_uri(default_storage.url(saved_path))
            )
        ...
        if serializer.is_valid():
            ...
            response_serializer = AnnouncementSerializer(
                announcement, context={"request": request, "user": user}
            )
            return Response(
                {
...
class ObtainAnnouncementsView(APIView):
    def get(self, request, clase_id=None):
        ...
        announcements = (
            Announcement.objects.filter(clase=clase)
            .annotate(
                comments_count_annotated=Count("comments", distinct=True),
                last_comment_at=Max("comments__created_at"),
            )
            .annotate(
                last_activity_at_annotated=Coalesce("last_comment_at", "updated_at")
            )
        )
        ...
        if sort_mode == "new":
            ...
            announcements = announcements.order_by(
                "-comments_count_annotated", "-last_activity_at_annotated"
            )
        else:
            announcements = announcements.order_by(
                "-last_activity_at_annotated", "-created_at"
            )

        serializer = AnnouncementSerializer(
            announcements, many=True, context={"request": request, "user": user}
        )
        return Response(
...
class ObtainAnnouncementDetailView(APIView):
    def get(self, request, announcement_id):
        ...
        try:
            ...
            return Response(
                {"Error": "Anuncio no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        comments = (
            AnnouncementComment.objects.filter(announcement=announcement)
        ...
        comments_data = AnnouncementCommentSerializer(
            comments, many=True, context={"request": request, "user": user}
        ).data
...
class ManageAnnouncementView(APIView):
    ...
    def patch(self, request, announcement_id):
        ...
        if "urls" in raw_data:
            data["urls"] = [
                item
                for item in parse_list(raw_data.get("urls"))
        ...
        if "photos" in raw_data:
            existing_photos = [
                item
                for item in parse_list(raw_data.get("photos"))
                if isinstance(item, str)
            ]
        else:
        ...
        response_serializer = AnnouncementSerializer(
            updated, context={"request": request, "user": user}
        )
        return Response(
            {
...
class CreateAnnouncementCommentView(APIView):
    def post(self, request, announcement_id):
        ...
        try:
            announcement = Announcement.objects.select_related("clase").get(
                id=announcement_id
            )
        except Announcement.DoesNotExist:
            return Response(
        ...
        if not content:
            return Response(
                {"Error": "content es requerido"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if parent_id not in (None, ""):
            try:
                parent_comment = AnnouncementComment.objects.get(
                    id=parent_id, announcement=announcement
                )
        ...
        serializer = AnnouncementCommentSerializer(
            comment, context={"request": request, "user": user}
        )
        return Response(
...
class ManageAnnouncementCommentView(APIView):
    def patch(self, request, comment_id):
        ...
        try:
            comment = AnnouncementComment.objects.select_related(
                "announcement__clase", "author"
            ).get(id=comment_id)
        except AnnouncementComment.DoesNotExist:
            return Response(
...
class CreateTaskView(APIView):
    ...
    def post(self, request, clase_id=None):
        ...
        allow_any_file_type = parse_bool(
            raw_data.get("allow_any_file_type"), default=True
        )
        ...
        serializer = TaskSerializer(
            data=data, context={"user": user, "request": request, "clase": clase}
        )
        if not serializer.is_valid():
            return Response(
                {"Error": "Datos inválidos", "details": serializer.errors},
...
class ManageTaskView(APIView):
    ...
    def patch(self, request, task_id):
        ...
        if "allow_any_file_type" in raw_data:
            allow_any_file_type = parse_bool(
                raw_data.get("allow_any_file_type"), default=task.allow_any_file_type
            )
        ...
        if "allowed_extensions" in raw_data:
            allowed_extensions = normalize_extensions(
                raw_data.get("allowed_extensions")
            )
        ...
        response_data.update(
            build_teacher_task_participants(updated_task, request=request)
        )
        return Response(
...
class ObtainPendingTasksView(APIView):
    def get(self, request, clase_id):
        ...
        tasks = list(
            get_pending_tasks_queryset(clase).prefetch_related(
                "submissions__files", "submissions__student"
            )
        )
        ...
        if user_is_teacher:
            ...
            return Response(
                {"is_teacher": True, "clase_id": clase_id, "tasks": tasks_data},
                status=status.HTTP_200_OK,
            )
        ...
        return Response(
            {"is_teacher": False, "clase_id": clase_id, "tasks": tasks_data},
            status=status.HTTP_200_OK,
        )
...
class ObtainTaskDetailView(APIView):
    def get(self, request, task_id):
        ...
        try:
            task = (
                Task.objects.select_related("clase")
        ...
        if user_is_teacher:
            ...
            return Response(
                {"is_teacher": True, "task": base_task}, status=status.HTTP_200_OK
            )

        submission = (
            TaskSubmission.objects.filter(task=task, student=user)
            .prefetch_related("files")
            .first()
        )
        submission_data = None
        if submission:
            submission_data = TaskSubmissionSerializer(
        ...
        return Response(
            {"is_teacher": False, "task": response_data}, status=status.HTTP_200_OK
        )
...
class GradeTaskSubmissionView(APIView):
    def post(self, request, task_id):
        ...
        if not is_teacher_in_class(user, task.clase, membership=membership):
            return Response(
                {"Error": "Solo el profesor puede calificar"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if not student_id:
            return Response(
                {"Error": "student_id es requerido"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            student_membership = ClaseMembership.objects.select_related("user").get(
                clase=task.clase, user_id=student_id
            )
        except ClaseMembership.DoesNotExist:
            return Response(
                {"Error": "El usuario no pertenece a esta clase"},
                status=status.HTTP_404_NOT_FOUND,
            )

        if student_membership.role in ("teacher", "assistant"):
            return Response(
                {"Error": "No se puede calificar a un miembro del profesorado"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"Error": "El alumno aún no ha entregado esta tarea"},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        student_summary = next(
            (
                ...
                if item["id"] == student_membership.user_id
            ),
            None,
        )
        task_summary = next(
            (item for item in dashboard["tasks"] if item["id"] == task.id), None
        )
        return Response(
            {
